...
print('Welcome', User)
EncodingInput = input('enter your string to encode')
...
def EncodingBase():
    MessageSplit = list(EncodingInput)
    OutputMessage = []
    MessageReOrder = []
    PacketOutput = []
    OutputTotal = 0
    ...
    if 'b' in MessageSplit:
      ...
      # RMposition = MessageSplit.remove(Wposition, CharW)
        ...
        try:
            for CountCharB in range(0, MessageCount):
                Bposition = MessageSplit.index('b')
                OutputTotal += 4
    ...
    elif print('either you have not input a character,interger or symbol'):
        sys.exit()
...
#bliss is fat         
    ...
    #old output shuffle
    #Random order output encoding
    #PacketOrder = [*range(0,MessageCount)]
    #ShuffleOrder = random.sample(PacketOrder, len(PacketOrder))
    
    #print(ShuffleOrder)
    
    #key testing/encoding
    OutputTotalPacket = []
    OutputTotalPacket.append(str(OutputTotal))
    OutputOrder = []
    MessageReShuffle = random.sample(MessageReOrder, len(MessageReOrder))
    print(MessageReShuffle)
    
    key = str(MessageReOrder[0])
    
    if key in OutputTotalPacket:
        print('key recieved')
    else:
        print('key declined')
